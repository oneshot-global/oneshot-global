...
- **通常モード（/upload）のフロントタイムアウト25秒 vs 本番実測最大47秒**: 遅いリクエストはクライアント側で先に AbortError になり（現状はページリロード）、サーバー側では処理が完走している可能性がある。**%表示化によりタイムアウトの体感がより目立つ可能性がある**（%が進んでいる途中で突然リロードされる）。タイムアウト延長 or リトライ導線は将来の課題（2026-07-06記録）
